...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP32Client_TeamB")) { //"ESPClient_3" represent the client name that connects to broker
      Serial.println("connected");
      // Subscribe
      client.subscribe("TeamB_sub");          //SUBSCRIBE topic
    } else {
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    delay(500);               // delay
      ...
      client.publish("TeamB", tempString);      //PUBLISH topic
