...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP32Client_TeamF")) { //"ESPClient_3" represent the client name that connects to broker
      Serial.println("connected");
      // Subscribe
      client.subscribe("TeamF_sub");          //SUBSCRIBE topic
    } else {
